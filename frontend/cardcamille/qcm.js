(() => {
  ...
  function renderQcm(records, headers) {
    ...
    records.forEach((record, index) => {
      ...
      const questionState = {
        ...
        footer: node.querySelector('.qcm-footer'),
      };
      ...
      if (!choices.length) {
        ...
        choices.forEach((choice, choiceIndex) => {
          ...
          button.type = 'button';
          button.classList.add('qcm-choice-btn');
          ...
          markerSpan.classList.add('qcm-choice-marker');
          markerSpan.textContent = marker;

          const textSpan = document.createElement('span');
          textSpan.classList.add('qcm-choice-text');
          textSpan.textContent = choice.label;

          button.appendChild(markerSpan);
          button.appendChild(textSpan);
          button.addEventListener('click', () => handleChoiceSelect(index, choiceIndex));

          listItem.appendChild(button);
          choiceList.appendChild(listItem);

          questionState.choiceElements.push({ listItem, button });
        });
      }
      ...
      questionState.footer.innerHTML = '';
  ...
  function handleChoiceSelect(questionIndex, choiceIndex) {
    ...
    questionState.choiceElements.forEach(({ button }) => {
      button.classList.remove('qcm-choice-btn--selected');
    });

    choiceEntry.button.classList.add('qcm-choice-btn--selected');
  ...
  function revealAnswers() {
    ...
    questionStates.forEach((questionState) => {
      ...
      choiceElements.forEach(({ button }) => {
        button.classList.remove('qcm-choice-btn--correct', 'qcm-choice-btn--incorrect');
      });
      ...
      choiceElements.forEach(({ button }, idx) => {
        ...
        if (isCorrect) {
          button.classList.add('qcm-choice-btn--correct');
        }

        if (selectedIndex === idx && !isCorrect && hasCorrectValue) {
          button.classList.add('qcm-choice-btn--incorrect');
        }
      });
