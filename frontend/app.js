(() => {
  ...
  csvInput.addEventListener('change', async (event) => {
    ...
    try {
      ...
      setStatus(`Import réussi : ${result.records.length} carte(s) depuis « ${file.name} ». Cartes mélangées automatiquement.`, 'success');
      toggleRenderButton();
  ...
  function renderCards(records, headers) {
    ...
    // Mélanger les cartes après l'import
  ...
  function flipCurrentCard(forceState) {
    ...
    const cardFlip = cardsContainer.querySelector('.card-flip');
    if (cardFlip) {
      cardFlip.classList.toggle('flipped', isCardFlipped);
    }
    updateFlipButtonLabel();
  }
